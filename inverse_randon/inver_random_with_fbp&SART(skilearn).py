...
k_ = 100 # angles = image.shape - k_
theta = np.linspace(0., 180., max(image.shape)-k_, endpoint=False)
sinogram = radon(image, theta=theta)
dx, dy = 0.5 * 180.0 / (max(image.shape)-k_), 0.5 / sinogram.shape[0]
